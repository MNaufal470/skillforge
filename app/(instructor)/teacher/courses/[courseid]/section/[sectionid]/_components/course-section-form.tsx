...
import ChapterLock from "./chapter-lock";

interface SectionChapterFormProps {
  initialData?: Chapter[];
  courseid: string;
  sectionid: string;
  isPublished: boolean;
...
const SectionChapterForm = ({
  initialData,
  courseid,
  sectionid,
  isPublished,
}: SectionChapterFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await axios.post(
        `/api/course/${courseid}/section/${sectionid}/chapter`,
        values
      );
      setIsCreating(false);
  ...
  const onReorder = async (updateData: { id: number; position: number }[]) => {
    try {
      setIsUpdating(true);
      await axios.put(`/api/course/${courseid}/section/${sectionid}/chapter/`, {
        list: updateData,
      });
  ...
  return (
    ...
    >
      ...
      {!isCreating && !isPublished && (
        ...
        >
          {!initialData?.length && "Chapters has not been set yet."}
...
export default SectionChapterForm;
